...
function reorganizeContentForPDF(clonedContent) {
    try {
        ...
        if (!personalInfo || !skills || !certificates || !experiences) {
            console.error('找不到必要的區塊元素');
            return;
        }
        ...
        console.log('PDF內容重組完成');
        
    } catch (error) {
        console.error('PDF內容重組失敗:', error);
        // 如果重組失敗，就使用原始內容
    }
}